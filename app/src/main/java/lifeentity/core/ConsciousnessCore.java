...
public class ConsciousnessCore {
   ...
   private void saveToLongTermMemory() {
       ...
       // نسخة آمنة من القائمة لتجنب التعديل أثناء التكرار
       List<ConsciousMoment> momentsToSave;
       synchronized (significantMomentsBuffer) {
           momentsToSave = new ArrayList<>(significantMomentsBuffer);
           significantMomentsBuffer.clear();
       }
       
       memoryExecutor.execute(() -> {
           for (ConsciousMoment moment : momentsToSave) {
               try {
                   EpisodicMemory.EventEntity event = new EpisodicMemory.EventEntity();
                   ...
                   Log.e(TAG, "Error saving moment to database", e);
               }
           }
           Log.d(TAG, "Saved " + momentsToSave.size() + " moments to long-term memory");
       });
   }
