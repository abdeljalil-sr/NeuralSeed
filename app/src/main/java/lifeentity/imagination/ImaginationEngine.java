...
public class ImaginationEngine {
   ...
   /**
    * توليد صورة مع دمج السياق (المفهوم الأساسي + العاطفة + الذكريات المرتبطة)
    */
   public Bitmap generateImageWithContext(String baseConcept, EmotionalState emotion, List<String> relatedMemories) {
       float[] latent = conceptToLatent(baseConcept);
       
       // تعديل المتجه حسب العاطفة
       if (emotion != null) {
           float[] affect = emotion.toAffectVector();
           for (int i = 0; i < Math.min(5, latent.length); i++) {
               latent[i] += affect[i] * 0.2f;
           }
       }
       
       // إضافة تأثير الذاكرة (دمج متجهات ذكريات مشابهة)
       if (relatedMemories != null && !relatedMemories.isEmpty()) {
           float[] memoryBlend = new float[LATENT_SIZE];
           int count = 0;
           for (String mem : relatedMemories) {
               float[] memLatent = conceptToLatent(mem);
               for (int i = 0; i < LATENT_SIZE; i++) {
                   memoryBlend[i] += memLatent[i];
               }
               count++;
           }
           if (count > 0) {
               for (int i = 0; i < LATENT_SIZE; i++) {
                   memoryBlend[i] /= count;
                   latent[i] = (latent[i] + memoryBlend[i]) / 2;
               }
           }
       }
       
       normalizeVector(latent);
       return generateImageFromLatent(latent);
   }
